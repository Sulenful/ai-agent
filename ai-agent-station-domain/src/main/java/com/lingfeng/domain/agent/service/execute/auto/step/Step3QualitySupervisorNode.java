 ...
 * @author: LingFeng Su
 * @version: V1.0
 * @date: 2026/1/20 16:22
 * @description:
 */
@Service
@Slf4j(topic = "Step3QualitySupervisorNode")
    ...
    private void parseSupervisionResult(DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext, String supervisionResult, String sessionId) {
        ...
        for (String line : lines) {
            ...
            if (line.contains("质量评估:")) {
                ...
                currentSection = ASSESSMENT.getSubType();
                ...
                // 发送前一个部分的内容
                sendSupervisionSubResult(dynamicContext, currentSection, sectionContent.toString(), sessionId);
                currentSection = ISSUES.getSubType();
                sectionContent.setLength(0);
                log.info("\n⚠️ 问题识别:");
                continue;
                ...
                // 发送前一个部分的内容
                sendSupervisionSubResult(dynamicContext, currentSection, sectionContent.toString(), sessionId);
                currentSection = SUGGESTIONS.getSubType();
                sectionContent.setLength(0);
                log.info("\n💡 改进建议:");
                continue;
                ...
                // 发送前一个部分的内容
                sendSupervisionSubResult(dynamicContext, currentSection, sectionContent.toString(), sessionId);
                currentSection = SCORE.getSubType();
                sectionContent.setLength(0);
                ...
                // 发送前一个部分的内容
                sendSupervisionSubResult(dynamicContext, currentSection, sectionContent.toString(), sessionId);
                currentSection = PASS.getSubType();
                sectionContent.setLength(0);
